...
def uniformCostSearch(problem):
    ...
    while fringe.isEmpty()==0:
        ...
        if curState not in path:
            path.add(curState)
            for n in problem.getSuccessors(curState):
                state = n[0]
                direction=n[1]
                if state not in path:
                    fringe.push((state, curMoves + [direction]), problem.getCostOfActions(curMoves+[direction]) ) 
    ...
    while fringe.isEmpty() == 0:
        ...
        if curState not in path:
            ...
            for n in problem.getSuccessors(curState):
                ...
                direction=n[1]
                if state not in path:
                    fringe.update((state, curMoves + [direction]), problem.getCostOfActions(curMoves+[direction])+heuristic(state, problem) ) 
